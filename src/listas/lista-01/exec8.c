...
struct tipoData lerData(const char* mensagem) {
    ...
    // Convertendo o dia (índices 0 e 1)
    aux[0] = dataLida[0];
    aux[1] = dataLida[1];
    aux[2] = '\0';
    d.dia = atoi(aux);
    
    // Convertendo o mês (índices 3 e 4)
    aux[0] = dataLida[3];
    aux[1] = dataLida[4];
    aux[2] = '\0';
    d.mes = atoi(aux);
    
    // Convertendo o ano (índices 6 a 9)
    aux[0] = dataLida[6];
    aux[1] = dataLida[7];
    aux[2] = dataLida[8];
    aux[3] = dataLida[9];
...
int main() {
    ...
    if (resultado < 0) {
        printf("\nA primeira data (%02d/%02d/%04d) ocorre primeiro.\n", data1.dia, data1.mes, data1.ano);
    } else if (resultado > 0) {
        printf("\nA segunda data (%02d/%02d/%04d) ocorre primeiro.\n", data2.dia, data2.mes, data2.ano);
    } else {
        printf("\nAs duas datas são iguais (%02d/%02d/%04d).\n", data1.dia, data1.mes, data1.ano);
    }
